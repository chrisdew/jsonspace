...
if (false) {
  URL0 = 'ws://localhost:8888/pubsub';
  URL1 = 'ws://localhost:8888/pubsub';
  URL2 = 'ws://localhost:8888/pubsub';
} else {
  URL0 = 'ws://45.79.6.86:8888/pubsub';
  URL1 = 'ws://106.186.121.210:8888/pubsub';
  URL2 = 'ws://106.186.121.210:8888/pubsub';
}
...
describe('pubsub', function() {
  it('should work for a simple two user test case', function (done) {
    ...
    const steps = [
      ...
      () => conn_a = new WrappedWebSocket('conn_a', URL0, parallel()),
      () => conn_b = new WrappedWebSocket('conn_b', URL1, next()),
      ...
      //() => setTimeout(next(), 1000), // time to allow replication
...
describe('watch', function() {
  it('should work', function (done) {
    ...
    const steps = [
      // make two separate websocket connections to the server
      () => conn_c = new WrappedWebSocket('conn_c', URL0, parallel()),
      () => conn_d = new WrappedWebSocket('conn_d', URL1, next()),
...
describe('unsubscribe', function() {
  it('should work', function(done) {
    ...
    const steps = [
      // make two separate websocket connections to the server
      () => conn_e = new WrappedWebSocket('conn_e', URL0, parallel()),
      () => conn_f = new WrappedWebSocket('conn_f', URL1, next()),
...
describe('unwat_ch', function() {
  it('should work', function (done) {
    ...
    const steps = [
      // make two separate websocket connections to the server
      () => conn_g = new WrappedWebSocket('conn_e', URL0, parallel()),
      () => conn_h = new WrappedWebSocket('conn_f', URL1, next()),
...
describe('multisub', function() {
  it('should work', function(done) {
    ...
    const steps = [
      // make two separate websocket connections to the server
      () => conn_i = new WrappedWebSocket('conn_i', URL0, parallel()),
      () => conn_j = new WrappedWebSocket('conn_j', URL1, parallel()),
      () => conn_k = new WrappedWebSocket('conn_k', URL2, next()),
...
describe('updated_extra', function() {
  it('should work', function (done) {
    ...
    const steps = [
      // make two separate websocket connections to the server
      () => conn_l = new WrappedWebSocket('conn_l', URL0, parallel()),
      () => conn_m = new WrappedWebSocket('conn_m', URL1, parallel()),
      () => conn_n = new WrappedWebSocket('conn_n', URL2, next()),
  ...
  describe('published_since', function() {
    it('should cause old message to be sent when subscribing', function (done) {
      ...
      const steps = [
        // make two separate websocket connections to the server
        () => conn_o = new WrappedWebSocket('conn_o', URL0, next()),
        ...
        () => conn_p = new WrappedWebSocket('conn_p', URL1, next()),
        () => conn_p.send({
          subscribe: {
            username: 'user_p',
            channel: '#channel_7',
